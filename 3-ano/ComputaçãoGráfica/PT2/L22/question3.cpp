...
void drawScene(void)
{
    ...
    //Construindo uma sépala e adicionando a lista: base + 1
    // Sepal display list.
    glNewList(base + 1, GL_COMPILE);
        glColor3f(1.0, 0.0, 0.0);
        glPushMatrix();
            glRotatef(90.0, 0.0, 0.0, 1.0);
            // animação de aumentar a sépala
    ...
    //Construindo uma pétala e adicionando a lista: base + 2
    ...
    glNewList(base + 2, GL_COMPILE);
        ...
        glPushMatrix();
            ...
            glRotatef(petalOpenAngle, 0.0, 1.0, 0.0); //animação de abrir as petalas
            glTranslatef(2.0, 0.0, 0.0);

            // Circle is squashed to an ellipse.
            glScalef(1.0, petalAspectRatio, 1.0); //animação de crescimento da petalas
    ...
    // First stem segment.
    glRotatef(angleFirstSegment, 0.0, 0.0, 1.0); //animação
    glCallList(base);
    ...
    // Sepal.
    /*Inicio da modelagem da sépala*/
    glTranslatef(7.0, 0.0, 0.0);
    glCallList(base + 1);
    /*fim da modelagem da sépala*/

    /*Inicio da modelagem das pétalas*/
    // First petal.
    glPushMatrix();
