...
//
// In develop mode:
// Press the space bar to select a part.  -> para selecionar uma parte do corpo
// Press the 'page up'/'page down' keys to rotate the selected part. -> para rotacionar a parte do corpo
// Press the left/right/up/down arrow keys to move the whole configuration. -> move o personagem
// Press r/R to rotate the viewpoint. -> rotaciona a camera
// Press z/Z to zoom in/out.  -> zoom
...
void Man::writeData()
{
   ...
   whatMan == true ? what = 1 : what = 2;

   floatToString(buffer, 4, what);	
   glRasterPos3f(-35.0, 22, -10.0);
   writeBitmapString((void*)font, "Man = "); 
   glRasterPos3f(-18.0, 22, -10.0);
   writeBitmapString((void*)font, buffer);

   floatToString(buffer, 4, partAngles[0]);	
   glRasterPos3f(-35.0, flag, -10.0);
   writeBitmapString((void*)font, "torso = "); 
   glRasterPos3f(-18.0, flag, -10.0);
   writeBitmapString((void*)font, buffer);
   ...
   glRasterPos3f(-35.0, flag, -10.0);
   writeBitmapString((void*)font, "left hand = "); 
   glRasterPos3f(-18.0, flag, -10.0);
   writeBitmapString((void*)font, buffer);
   ...
   glRasterPos3f(-35.0, flag, -10.0);
   writeBitmapString((void*)font, "right left = "); 
   glRasterPos3f(-18.0, flag, -10.0);
   writeBitmapString((void*)font, buffer);
   ...
   glRasterPos3f(-35.0, flag, -10.0);
   writeBitmapString((void*)font, "left foot = "); 
   glRasterPos3f(-18.0, flag, -10.0);
   writeBitmapString((void*)font, buffer);
   ...
   glRasterPos3f(-35.0, flag, -10.0);
   writeBitmapString((void*)font, "right foot = "); 
   glRasterPos3f(-18.0, flag, -10.0);
   writeBitmapString((void*)font, buffer);
...
void drawScene(void)
{
   ...
   vector<Man>::iterator localManVector2Iterator;
   ...
   if (!animateMode) // Develop mode.
   {
      ...
      localManVector2Iterator = manVector2.begin();
      ...
      while(localManVector2Iterator != manVector2.end() )
	   {
         localManVector2Iterator->draw();
	      localManVector2Iterator++;
	   }
   }
   else // Animated mode - 
   ...
   vector<Man>::iterator localManVectorIterator2;
   vector<Man>::iterator localManVectorIterator22;
   if (!animateMode) // Develop mode.
   {
      
      localManVectorIterator2 = manVector.end() - 1;
      localManVectorIterator22 = manVector2.end() - 1;
      gluLookAt(0.0, 11.5 + localManVectorIterator2->getUpMove(), localManVectorIterator2->getForwardMove(),
                30.0, 8.0, localManVectorIterator22->getForwardMove(),
                0.0, 1.0, 0.0);
   ...
   if (!animateMode) // Develop mode.
   {
      ...
      localManVectorIterator2 = manVector.end() - 1;
      localManVectorIterator22 = manVector.end() - 1;
      gluLookAt(0.0, 17.0 + localManVectorIterator22->getUpMove(), localManVectorIterator2->getForwardMove(),
                -30.0, 10.0, localManVectorIterator2->getForwardMove(),
                0.0, 1.0, 0.0);
...
void outputConfigurations(void)
{
   ...
   localManVectorIterator2 = manVector2.begin();
   while(localManVectorIterator2 != manVector2.end() )
   {
      localManVectorIterator2->outputData();
	  localManVectorIterator2++;
   }
   outFile.close();
}
...
void keyInput(unsigned char key, int x, int y)
{
   ...
   switch(key) 
   {
      ...
      case 'm': // Create new man configuration.
		 // Turn highlight off current configuration.
         manVectorIterator2->setHighlight(0); 

		 localManVectorIterator2 = manVectorIterator2;
		 manVectorIterator2++;

		 // Insert copy of current configuration and highlight.
         manVectorIterator2 = 
		   manVector2.insert(manVectorIterator2, Man(*localManVectorIterator2) );
         manVectorIterator2->setHighlight(1);
      ...
      case ' ': // Select next body part.
         if (whatMan)
            ...
            manVectorIterator2->incrementSelectedPart();
	   ...
	   case 9:
         if (whatMan) {
            ...
            // Increment iterator - go to start if at end already.
            manVectorIterator2++;
               if (manVectorIterator2 == manVector2.end() ) 
               manVectorIterator2 = manVector2.begin();
	   ...
	   case 8: 
         if (whatMan) {
            ...
            if (manVectorIterator2 != manVector2.begin() ) // Not first configuration. 
            {
               // Make copy of the previous configuration and highlight.
               localManVectorIterator2 = manVectorIterator2;
               localManVectorIterator2--;
               ...
               // Delete current configuration.
               manVectorIterator2++;
               manVectorIterator2 = manVector2.erase(manVectorIterator2);
            ...
            else // First configuration
            {
               ...
               // Create new configuration.
               manVector2.insert(manVectorIterator2, Man() );
            }
         }
		 glutPostRedisplay();
		 break;
	   ...
	   case 127:
         if (whatMan) {
            ...
            if (manVector2.size() > 1) 
            {
               manVectorIterator2 = manVector2.erase(manVectorIterator2);
               if (manVectorIterator2 != manVector2.begin() ) 
                  manVectorIterator2--;
...
{  
   if (whatMan) {
      ...
      if (key == GLUT_KEY_PAGE_DOWN) manVectorIterator2->decrementPartAngle();
      if (key == GLUT_KEY_PAGE_UP) manVectorIterator2->incrementPartAngle();
      if (key == GLUT_KEY_LEFT) manVectorIterator2->decrementForwardMove();
      if (key == GLUT_KEY_RIGHT) manVectorIterator2->incrementForwardMove();
      if (key == GLUT_KEY_DOWN)
      {
         if (!animateMode) manVectorIterator2->decrementUpMove();
      else animationPeriod +=10;
      }
      if( key == GLUT_KEY_UP)
      {
         if (!animateMode) manVectorIterator2->incrementUpMove();
      else if (animationPeriod > 10) animationPeriod -=10;
      }
   }
   glutPostRedisplay();
...
void printInteraction(void)
{
   ...
   cout << "Press a to toggle between develop and animate modes." << endl
		...
		<< "Press n (first person) and m (second person) to create a new configuration - other configurations are ghosted" << endl
      ...
      << endl << "Press 'q' to select the man" << endl;
}
