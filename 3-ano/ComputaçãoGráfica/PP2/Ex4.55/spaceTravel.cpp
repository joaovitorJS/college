...
#define ROW_BIG_GOLDEN 3
#define COLUMN_BIG_GOLDEN 3
#define COLLISION_BIG_GOLDEN 100
...
static int colorGold[3] = {255, 215, 0};
static bool glowAnimate = false;
                   ...
                   unsigned char colorG, unsigned char colorB)
{
    ...
    color[3] = 255; // só muda esse valor quando quer produzir o efeito brilhar no asteroide "Big Golden" 
}
...
//modifica a posição color[3];
void Asteroid::setColorAlpha(int a) {
    this->color[3] = a;
}

//retorna o valor do alpha
int Asteroid::getColorAlpha() {
    return this->color[3];
}
        ...
        for (i = 0; i < ROWS; i++)
            ...
            // which indicates that there is no asteroid there because the default's radius is 0.
            {
                ...
                if (COLUMNS % 2) { // Odd number of columns.
                    if (j == COLUMN_BIG_GOLDEN && i == ROW_BIG_GOLDEN) {
                        arrayAsteroids[i][j] = Asteroid(30.0 * (-COLUMNS / 2 + j), 0.0, -40.0 -30.0 * i, 6.0, 
                                                    colorGold[0], colorGold[1], colorGold[2]);
                    } else {
                    ...
                    if (j == COLUMN_BIG_GOLDEN && i == ROW_BIG_GOLDEN) {
                        arrayAsteroids[i][j] = Asteroid(15 + 30.0 * (-COLUMNS / 2 + j), 0.0, -40.0 -30.0 * i, 6.0,
                                                    colorGold[0], colorGold[1], colorGold[2]);
                    } else {
...
int asteroidCraftCollision(float x, float z, float a)
{
        ...
        for (i = 0; i < ROWS; i++)
            if (arrayAsteroids[i][j].getRadius() > 0) // If asteroid exists.
                if (checkSpheresIntersection(x - 5 * sin((PI / 180.0) * a), 0.0,
                    ...
                    if (j == COLUMN_BIG_GOLDEN && i == ROW_BIG_GOLDEN) {
                        //colidiu com o asteroide "Big Golden"
                        return COLLISION_BIG_GOLDEN;
                    } else {
                        return 1;
                    }
                }
 ...
 // faz o asteroide "Big Golden" 'brilhar' (oscilar sua cor)
void makeGlow(int v) {
    int glowAux = arrayAsteroids[ROW_BIG_GOLDEN][COLUMN_BIG_GOLDEN].getColorAlpha();
    ...
    if (!glowAnimate) {
        glowAux -= 6;
        arrayAsteroids[ROW_BIG_GOLDEN][COLUMN_BIG_GOLDEN].setColorAlpha(glowAux);
    } else {
        glowAux+= 6;
        arrayAsteroids[ROW_BIG_GOLDEN][COLUMN_BIG_GOLDEN].setColorAlpha(glowAux);
    }
...
void drawScene(void)
{
    ...
    else if (isCollision == COLLISION_BIG_GOLDEN)
        writeBitmapString((void *)font, (char *)"You have found gold!");
    
    glPopMatrix();

    // fixa a camera no centro do asteroid "Big Golden" e faz ele apontar para a espacionave.
    gluLookAt(arrayAsteroids[ROW_BIG_GOLDEN][COLUMN_BIG_GOLDEN].getCenterX(),
              arrayAsteroids[ROW_BIG_GOLDEN][COLUMN_BIG_GOLDEN].getCenterY(),
              arrayAsteroids[ROW_BIG_GOLDEN][COLUMN_BIG_GOLDEN].getCenterZ(),
              xVal, 0, zVal,
              0, 1.0, 0);
...
void specialKeyInput(int key, int x, int y)
{
    ...
    int aux = asteroidCraftCollision(tempxVal, tempzVal, tempAngle);
    ...
    if (aux == 0)
    {
        ...
        if(aux == 1)
            ...
            isCollision = COLLISION_BIG_GOLDEN;
